...
* actor[1].type = #entity
* actor[1].name = "Network or Intermediary Facilitating Data Exchange"
* actor[1].description = "An optional interoperability partner that can facilitate the exchange of bedside screening results for early hearing detection and intervention (EHDI) and/or critical congenital heart disease (CCHD) between a healthcare organization and an appropriate Public Health Agency."
* actor[2].actorId = "HealthcareOrganization"
* actor[2].type = #entity
* actor[2].name = "Healthcare Organization (HCO)"
* actor[2].description = "Healthcare organizations perform critical roles in collecting and sharing clinical and administrative data critical to public health program activities and goals."
* instance[0].resourceId = "patientQuery1"
* instance[0].resourceType = #Binary
* instance[0].name = "Query for Patient Record via an Intermediary"
* instance[0].description = "The PHA shares patient demographics to identify the appropriate individual of interest via an Intermediary."
...
* instance[2].description = "The Intermediary shares patient demographics provided by the PHA with an HCO to identify the appropriate individual of interest."
* instance[3].resourceId = "patientQueryResponse2"
* instance[3].resourceType = #Bundle
* instance[3].name = "Response to Query for Patient Record to an Intermediary"
* instance[3].description = "The HCO returns one or more Patient resources based on the provided demographics."
* instance[4].resourceId = "clinicalQuery1"
* instance[4].resourceType = #Binary
* instance[4].name = "Query for Clinical Data via an Intermediary"
* instance[4].description = "The PHA request additional data related to the identified Patient resource via an Intermediary. The exact nature of the query and data requested will vary based on the use case being fulfilled. PHAs only request data for which they have clear authority to access."
...
* instance[5].description = "The Intermediary returns appropriate data received from the HCO in response to the PHA query for additional information."
...
* instance[7].description = "The HCO returns appropriate data in response to the query for additional information."
* process[0].title = "Patient Identification Query"
...
* process[0].step[0].operation[0].number = "1"
* process[0].step[0].operation[0].type = "Patient Identification Query 1"
* process[0].step[0].operation[0].name = "Patient Identification Query via an Intermediary"
* process[0].step[0].operation[0].initiator = "PublicHealthAgency"
* process[0].step[0].operation[0].receiver = "Intermediary"
* process[0].step[0].operation[0].request.resourceId = "patientQuery1"
* process[0].step[0].operation[0].description = "The Public Health Agency generates a FHIR RESTful API query containing patient demographics to identify the appropriate Patient resource on the FHIR Server. The query is via an Intermediary which will handle the direct connection to the HCO FHIR Server."
* process[0].step[1].operation[0].number = "2"
* process[0].step[1].operation[0].type = "Patient Identification Query 2"
...
* process[0].step[1].operation[0].description = "The Intermediary forwards the patient demographics received from the PHA to the HCO in the form of a patient query. The HCO responses with one or more matching FHIR Patient resources."
* process[0].step[2].operation[0].number = "3"
* process[0].step[2].operation[0].type = "Patient Identification Response"
...
* process[1].step[0].operation[0].type = "Clinical Data Query 1"
...
* process[1].step[0].operation[0].description = "Using the identified Patient resource, the Public Health Agency formulates one or more use case specific FHIR RESTful API queries to collect additional relevant data and transmits them to an Intermediary."
* process[1].step[1].operation[0].number = "5"
* process[1].step[1].operation[0].type = "Clinical Data Query 2"
...
* process[1].step[1].operation[0].description = "The Intermediary forwards the queries received from the PHA to the HCO. The HCO responses with the appropriate data."
* process[1].step[2].operation[0].number = "6"
* process[1].step[2].operation[0].type = "Clinical Data Response"
