...
Under TEFCA, QHINs may provide a single point of connection for PHAs to reach out to the nationwide network of responders to facilitate the discovery of the endpoint with a record locator service, and a Directory of Requestors/Responders, and establish trust so the PHA can directly initiate a query, while following a federally supported set of standards. Record Locator Services need to be jurisdiction sensitive to access minimum necessary data as authorized so PHAs do not inadvertently pull data from other jurisdictions for the patient.